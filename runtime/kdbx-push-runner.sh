...
PUSH_EVENT_TOKEN="$STATE_DIR/push_event_token"
...
schedule_push() {
  ...
  printf '%s\n' "$TOKEN" >"$PUSH_EVENT_TOKEN"

  (
    sleep "$DEBOUNCE"

    local LAST
    LAST=$(cat "$PUSH_EVENT_TOKEN")
